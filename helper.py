...
def calculate_acceleration(gdf, spd_column='velocity', ts_column='ts'):
	...
	gdf.loc[:, 'acceleration'] = gdf[spd_column].diff(-1).divide(np.array(gdf[ts_column].diff(-1).abs().values, dtype=int)/1000000000)
...
def calculate_velocity(gdf, spd_column='velocity', ts_column='ts'):
	...
	gdf.loc[:, 'dt'] = np.array(gdf[ts_column].diff(-1).abs().values, dtype=int)/1000000000 # ns to sec
...
def create_radial_chart(ax, df, bins_number, degree_intervals):
	...
	plt.suptitle('Bus course polar chart', fontsize=8, y=1.02)
